...
- **期限**：政府レビューは約30日規模と報じられているが、これは主に各機関の実施期限であり、フロンティアモデルの分類・レビュー枠組み全体には60日の実装期限（8月1日前後）も存在する。GPT-5.6の一般提供時期について、OpenAI公式は「数週間以内」としか示しておらず、特定の期日は確定していない。
...
- 焦点はSolの攻撃的サイバーセキュリティ能力。政府レビューは約30日規模と報じられるが制度全体には60日期限（8月1日前後）もあり、一般提供時期はOpenAI公式が「数週間以内」と示すのみで期日は未確定
